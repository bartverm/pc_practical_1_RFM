...
In this case we have the same critical slope for both reaches:
    ...
    \lstinputlisting[linerange={32-44}]{matlab/river_engineering_exercises.m}
...
We determine first the slope type:
    ...
    In Matlab:
    \lstinputlisting[linerange={46-58}]{matlab/river_engineering_exercises.m}
...
\subsection{River bend cutoff}
  ...
  \begin{solution}
    We start by creating the initial situation:
    \lstinputlisting[linerange={7-23}]{matlab/bend_cutoff_example.m}
    \includegraphics[width=\linewidth]{matlab/cutoff_initial.pdf}
    
    We now modify the case to include the cutoff:
    \lstinputlisting[linerange={25-32}]{matlab/bend_cutoff_example.m}
    \includegraphics[width=\linewidth]{matlab/cutoff_final.pdf}
  \end{solution}
  ...
  \begin{solution}
    ...
    \lstinputlisting[linerange={34-36}]{matlab/bend_cutoff_example.m}
...
\begin{solution}
  ...
  \lstinputlisting[linerange={7-34}]{matlab/hydraulic_jump_example.m}
  
  We will now plot the two backwater curves:
  
  \lstinputlisting[linerange={36-48}]{matlab/hydraulic_jump_example.m}
...
\begin{solution}
  \lstinputlisting[linerange={50-80}]{matlab/hydraulic_jump_example.m}
  
  We can illustrate what we did by plotting the depth at which the upstream supercritical flow would jump into the plot with the subcritical downstream jump and highlight the intersection point:

  \lstinputlisting[linerange={82-86}]{matlab/hydraulic_jump_example.m}
  \includegraphics[width=\linewidth]{matlab/hyd_jump_intersect.pdf}
  
  Let's now plot the final result:
  \lstinputlisting[linerange={88-95}]{matlab/hydraulic_jump_example.m}
  \includegraphics[width=\linewidth]{matlab/hyd_jump_final.pdf}
\end{solution}
...
\begin{solution}
  We can re-use the script above and slightly modify the input parameters to move the hydraulic jump. For a move to the left (upstream) we can raise the weir to e.g. \si{6}{\cm} resulting in the following situation:
  \includegraphics[width=\linewidth]{matl}
\end{solution}
